...
import com.hy.wificonnect.adapter.WifiListAdapter;
import com.hy.wificonnect.app.AppContants;
import com.hy.wificonnect.bean.WifiBean;
import com.hy.wificonnect.dialog.WifiLinkDialog;
import com.hy.wificonnect.utils.CollectionUtils;
...
public class WifilistconnectActivity extends AppCompatActivity {

    private static final String TAG = "WifilistconnectActivity";
    //权限请求码
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wifilistconnect);
        pbWifiLoading = (ProgressBar) this.findViewById(R.id.pb_wifi_loading);

        hidingProgressBar();
        mHasPermission = checkPermission();
        if (!mHasPermission && WifiSupport.isOpenWifi(WifilistconnectActivity.this)) {  //未获取权限，申请权限
            requestPermission();
        }else if(mHasPermission && WifiSupport.isOpenWifi(WifilistconnectActivity.this)){  //已经获取权限
            initRecycler();
        }else{
            Toast.makeText(WifilistconnectActivity.this,"WIFI处于关闭状态", Toast.LENGTH_SHORT).show();
        }
    ...
    private void initRecycler() {
        ...
        if(WifiSupport.isOpenWifi(WifilistconnectActivity.this) && mHasPermission){
            sortScaResult();
        }else{
            Toast.makeText(WifilistconnectActivity.this,"WIFI处于关闭状态或权限获取失败22222", Toast.LENGTH_SHORT).show();
        }
            ...
            public void onItemClick(View view, int postion, Object o) {
                ...
                if(wifiBean.getState().equals(AppContants.WIFI_STATE_UNCONNECT) || wifiBean.getState().equals(AppContants.WIFI_STATE_CONNECT)){
                    ...
                    if(WifiSupport.getWifiCipher(capabilities) == WifiSupport.WifiCipherType.WIFICIPHER_NOPASS){//无需密码
                        WifiConfiguration tempConfig  = WifiSupport.isExsits(wifiBean.getWifiName(),WifilistconnectActivity.this);
                        if(tempConfig == null){
                            ...
                            WifiSupport.addNetWork(exsits, WifilistconnectActivity.this);
                        }else{
                            WifiSupport.addNetWork(tempConfig, WifilistconnectActivity.this);
                        }
        ...
        public void onReceive(Context context, Intent intent) {
            if(WifiManager.WIFI_STATE_CHANGED_ACTION.equals(intent.getAction())){
                ...
                if(NetworkInfo.State.DISCONNECTED == info.getState()){//wifi没连接上
                    ...
                    Log.d(TAG,"wifi连接上了");
                    hidingProgressBar();
                    WifiInfo connectedWifiInfo = WifiSupport.getConnectedWifiInfo(WifilistconnectActivity.this);

                    //连接成功 跳转界面 传递ip地址
                    Toast.makeText(WifilistconnectActivity.this,"wifi连接上了", Toast.LENGTH_SHORT).show();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PERMISSION_REQUEST_CODE) {
            ...
            if (hasAllPermission) {
                mHasPermission = true;
                if(WifiSupport.isOpenWifi(WifilistconnectActivity.this) && mHasPermission){  //如果wifi开关是开 并且 已经获取权限
                    initRecycler();
                }else{
                    Toast.makeText(WifilistconnectActivity.this,"WIFI处于关闭状态或权限获取失败1111", Toast.LENGTH_SHORT).show();
                }

            } else {  //用户不同意权限
                mHasPermission = false;
                Toast.makeText(WifilistconnectActivity.this,"获取权限失败", Toast.LENGTH_SHORT).show();
            }
        }
    }
